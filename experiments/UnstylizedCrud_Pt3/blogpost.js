export class blogPost{
    ...
    init(title, message, mock, recreateid, date){
        if(mock){
            ...
            this.post.innerHTML = `
            <section class="blogPost" id="post-${this.id}">
                ...
                <div class="editMenu">
                    <button id="deletepost${this.id}">Delete</button>
                    <button id="editpost${this.id}">Edit</button>
                    <hr>
                </div>
            </section>
            `;
...
export class postDialog{
    ...
    okHandler(){
        ...
        let txt = "";
        if(this.mode ==2){
            title = this.parentElement.querySelector(`#title${this.internalid}`).value;
            title = DOMPurify.sanitize(title);
            txt = this.parentElement.querySelector(`#message${this.internalid}`).value;
